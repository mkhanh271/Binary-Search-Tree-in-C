...
int main() {
    ...
    while (true) {
        cout << "Choose input mode ('int', 'string', or 'quit'): ";
        getline(cin, mode);
        if (mode == "quit") break;
        ...
        while (true) {
            ...
            if (operation == "exit") {
                cout << "Exiting " << (isIntMode ? "integer" : "string") << " mode...\n";
                break;
            }
            ...
            while (true) {
                ...
                if (isIntMode) {
                    try {
                        ...
                        if (operation == "add") {
                            iTree->add(v);
                            cout << "Added " << v << ". Tree updated:\n";
                            iTree->printTreeColored();
                        } else if (operation == "delete") {
                            if (iTree->contains(v)) {
                                iTree->deleteValue(v);
                                cout << "Deleted " << v << ". Tree updated:\n";
                                iTree->printTreeColored();
                            } else {
                                cout << "Can't delete " << v << " (not found).\n";
                            }
                        } else if (operation == "search") {
                            vector<int> indices = iTree->search(v);
                            if (indices.empty()) cout << "Value " << v << " not found.\n";
                            else {
                                cout << "Value " << v << " found at indices: ";
                    ...
                    if (operation == "add") {
                        sTree->add(val);
                        cout << "Added \"" << val << "\". Tree updated:\n";
                        sTree->printTreeColored();
                    } else if (operation == "delete") {
                        if (sTree->contains(val)) {
                            sTree->deleteValue(val);
                            cout << "Deleted \"" << val << "\". Tree updated:\n";
                            sTree->printTreeColored();
                        } else {
                            cout << "Can't delete \"" << val << "\" (not found).\n";
                        }
                    } else if (operation == "search") {
                        vector<int> indices = sTree->search(val);
                        if (indices.empty()) cout << "Value \"" << val << "\" not found.\n";
                        else {
                            cout << "Value \"" << val << "\" found at indices: ";
                            for (int idx : indices) cout << idx << " ";
                            cout << endl;
                        }
                    }
            ...
            cout << "\nOperation '" << operation << "' ended.";
        }
    }
    cout << "=== Final Trees ===\n";
    cout << "\nInteger Tree:\n";
    intTree.printTreeColored();
    cout << "\n=================================================\n";
    cout << "\nString Tree:\n";
    stringTree.printTreeColored();
    return 0;
}